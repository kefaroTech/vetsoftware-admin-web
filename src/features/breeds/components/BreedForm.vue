...
function emptyForm(): CreateBreedRequest {
  return { name: '', specieId: 0 }
}

const form = ref<CreateBreedRequest>(emptyForm())
...
// `CreateBreedRequest`. El `|| null` NO es adorno: `selection` solo trata como
// vacíos `null`, `undefined` y la cadena vacía, así que el 0 de «sin elegir»
// pasaría por válido y el formulario enviaría `specieId: 0` al servidor.
const errors = computed(() => ({
  name: length(form.value.name, 'El nombre de la raza', 2, 100),
