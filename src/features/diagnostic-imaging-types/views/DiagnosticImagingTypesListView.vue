...
const { diagnosticImagingTypes, loading, error, errorTraceId, fetchAll, create, remove } =
  useDiagnosticImagingTypes()
const { confirm } = useConfirmDialog()
const showModal = ref(false)
const saving = ref(false)
const formRef = ref<InstanceType<typeof DiagnosticImagingTypeForm> | null>(null)
 ...
 * respuesta, y `GET /diagnostic-imaging-types` devuelve `DiagnosticImagingTypeResponse[]` —el
 ...
 * dos lados de la comparación: con `toLowerCase().includes()`, «radiografia» no
 * encontraba «Radiografía», y en estos catálogos clínicos casi todos los nombres
 * llevan tilde. Vive en `@/composables/text` y NO se copia aquí.
 *
 * Se filtra sobre `diagnosticImagingTypes`, que es lo que el store guarda DESPUÉS del
...
const filtrados = computed(() =>
  diagnosticImagingTypes.value.filter((t) => coincide(q.value, t.name, t.description)),
)
...
<template>
  <AppLayout>
    ...
    <AppListSearch
      v-model="q"
      label="Buscar tipos de imagen diagnóstica"
    ...
    >
      <template #empty>
        ...
        <AppEmptyState
          v-else
          title="Aún no hay tipos de imagen diagnóstica"
          description="Agrupan los estudios de imagen —radiografía, ecografía— que las clínicas registran."
        >
          <button type="button" class="ds-btn ds-btn--primary" @click="showModal = true">
            <component :is="ICONS.ADD" :size="15" />
            Nuevo tipo
    ...
    <p v-if="!loading && !error && diagnosticImagingTypes.length > 0" class="ds-meta">
      Mostrando {{ filtrados.length }} de {{ diagnosticImagingTypes.length }}
    </p>

    <AppModal :open="showModal" title="Nuevo tipo de imagen diagnóstica" @close="handleClose">
