...
const { selected, loading, error, errorTraceId, fetchById, update } = useSurgeryTypes()
...
<template>
  <AppLayout>
    ...
    <AppEmptyState
      v-else-if="!loading"
      title="Ese tipo de cirugía ya no está"
      description="Puede haberse eliminado desde otra sesión, o el enlace apunta a un identificador que ya no existe."
    >
      <RouterLink :to="{ name: ROUTE_NAMES.SURGERY_TYPES_LIST }" class="ds-btn ds-btn--primary">
        Volver al listado
      </RouterLink>
    </AppEmptyState>
  </AppLayout>
