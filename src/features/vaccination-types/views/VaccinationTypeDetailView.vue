...
const { selected, loading, error, errorTraceId, fetchById, update } = useVaccinationTypes()
...
<template>
  <AppLayout>
    <div class="ds-detail-head">
      ...
      <h1 class="ds-title">Editar tipo de vacuna</h1>
    </div>
    ...
    <AppEmptyState
      v-else-if="!loading"
      title="Ese tipo de vacuna ya no está"
      description="Puede haberse eliminado desde otra sesión, o el enlace apunta a un identificador que ya no existe."
    >
      <RouterLink :to="{ name: ROUTE_NAMES.VACCINATION_TYPES_LIST }" class="ds-btn ds-btn--primary">
        Volver al listado
      </RouterLink>
    </AppEmptyState>
  </AppLayout>
